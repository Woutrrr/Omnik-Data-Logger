...
class MQTTOutput(PluginLoader.Plugin):
    ...
    def __init__(self):
        ...
        mqttuser = self.config.get('mqtt', 'mqttuser')
        mqttpasswd = self.config.get('mqtt', 'mqttpasswd')
        self.client.username_pw_set(username=mqttuser, password=mqttpasswd)
        
        mqtthost = self.config.get('mqtt', 'mqtthost')
        mqttport = self.config.get('mqtt', 'mqttport')
